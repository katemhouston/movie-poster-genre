...
def label_encoding(df):
    ...
    idx_to_label = {idx: genre for genre, idx in label_to_idx.items()}
...
def get_splits(df, random_state=42):
    try:
        train_val_df, test_df = train_test_split(
            df, test_size=0.1, stratify=df['primary_genre'], random_state=random_state
        )
        train_df, val_df = train_test_split(
            train_val_df, test_size=1/9, stratify=train_val_df['primary_genre'], random_state=random_state
        )
    except ValueError:
        # fall back to random split if not enough samples per class to stratify
